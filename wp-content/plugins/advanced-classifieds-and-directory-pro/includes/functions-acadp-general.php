 ...
 */
 function acadp_categories( $settings ) {	
	if ( $settings['depth'] <= 0 ) {
		return;
	}
	...
	if ( count( $terms ) > 0 ) {			
		...
		foreach ( $terms as $term ) {
			...
			//$html .= '<li>'; 
			$html .= '<a class="list-group-item list-group-item-action" href="' . acadp_get_category_page_link( $term ) . '" title="' . sprintf( __( "View all posts in %s", 'advanced-classifieds-and-directory-pro' ), $term->name ) . '" ' . '>';
			$html .= $term->name;
